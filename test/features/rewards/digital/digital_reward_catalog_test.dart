...
void main() {
  ...
  test('built-in catalog contains 30 unique functional rewards', () async {
    final rewards = await DigitalRewardCatalog.load();

    expect(rewards, hasLength(30));
    expect(rewards.map((reward) => reward.id).toSet(), hasLength(30));
    expect(
      rewards.map((reward) => reward.id),
      contains('mascot_victory_burst'),
    );
    expect(
